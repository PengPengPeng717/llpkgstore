...
Python 包现在使用与 C/C++ 包完全相同的版本管理机制，通过共享 `DefaultClient` 接口实现真正的统一。

#### 统一架构实现

- **共享接口**: Python 和 C/C++ 包都使用 `actions.NewDefaultClient()` 和 `client.Postprocessing()`
- **统一版本提取**: 使用相同的 `client.MappedVersion()` 方法
